...
test_DTM = as.data.frame(test_DTM)
```
...
author_rf = randomForest(classifiers~.,data=train_class, ntree=500, mtry=8,importance=TRUE)
...
## K-Nearest Neighbors
```{r KNN Model}
train_class = na.omit(train_class)
test_class = na.omit(test_class)
train_x = as.factor(subset(train_class, select = -c(classifiers)))
test_x = as.factor(subset(test_class,select=-c(results)))
classifiers = as.factor(train_class$classifier)

knn_preds = knn(train_x,test_x,classifiers,k=1)

temp = as.data.frame(cbind(results,knn_preds))
temp$tag = ifelse(temp$results == temp$knn_preds,1,0)
sum(temp$tag)*100/nrow(temp)
```
